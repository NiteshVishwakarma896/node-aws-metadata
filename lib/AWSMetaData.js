...
class AWSMetaData{
    ...
    async getInstanceMetaData (key){
        ...
        try {
            ...
            //     }
            // })
            const responseText = await makeHttpRequestAndReturnText(options.hostname, options.path);
            this.metadata = responseText; // Store the response data in the metadata property.
            return responseText; // Return the response data.
        } catch (error) {
            return error;
        }
    }
